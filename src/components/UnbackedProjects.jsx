...
const UnbackedProjects = ({ unbackedProjects, fromProject }) => {
  ...
  const getCollection = () => unbackedProjects.slice(0, end);

  useEffect(() => {
    setCollection(getCollection());
  }, [unbackedProjects, end]);

  return (
    <div className="flex flex-col px-6 mb-7">
      <div className="flex justify-center items-center flex-wrap">
        {collection.map((project, i) => (
          <ProjectCard key={i} project={project} fromProject={fromProject} />
        ))}
      </div>

      {unbackedProjects.length > collection.length ? (
...
const ProjectCard = ({ project, fromProject }) => {
...
export default UnbackedProjects;
